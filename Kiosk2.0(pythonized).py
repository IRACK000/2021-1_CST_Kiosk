# -*- coding: utf-8 -*-
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''
### Alias : Kiosk2.0.py & Last Modded : 2021.03.19. ###
...
class Kiosk():
    ...
    def boot(self):
        ...
        try:
            ...
            rdr = csv.reader(file)
            next(rdr)
            [self.phone.append(Phone(row[0], row[1], int(row[2]))) for row in rdr]
            #for row in rdr:
            #    self.phone.append(Phone(row[0], row[1], int(row[2])))  # 리스트 마지막에 Phone 클래스의 인스턴스 생성하여 추가
            file.close()
    ...
    def shutdown(self):
        ...
        [wr.writerow([stock.name, stock.stat, stock.price]) for stock in self.phone]
        #for stock in self.phone:
        #    wr.writerow([stock.name, stock.stat, stock.price])
        file.close()
        print("재고 목록을 저장했습니다.\n키오스크 OS를 종료합니다.")
    ...
    def passwdGen(self):
        ...
        return [[chrList[self.getPseudoRandNum(34)] for j in range(5)] for i in range(4)] #domains

    def passwdEecrypt(self, pidkey):  # PIDEY to ECDATA (한컴오피스 설치파일의 시리얼넘버 암호화 방식)
        ecdata = list()
        domain_code = [-3, 1, 2, -9]
        for domain in [1, 3, 0, 2]:
            for char in pidkey[domain]:
                if char >= 'A' and char <= 'Z':  # 대문자일 때
                    ecdata.append(chr((ord(char) - ord('A') + domain_code[domain] + 26) % 26 + ord('A')))
                else:  # 숫자 문자일 때
                    ecdata.append(chr((ord(char) - ord('0') + domain_code[domain] + 10) % 10 + ord('0')))
        return ecdata
    ...
    def printMainMenu(self):
        ...
        if self.option == "종료":
            ...
            if int(input("판매하시겠습니까?(Yes = 1, No = 0) : ")):
                self.phone.append(Phone(name, status, price))  # 리스트 마지막에 Phone 클래스의 인스턴스 생성하여 추가
            ...
            print("\n원하시는 기기는 재고가 없습니다.ㅠㅠ 3초 후 메인메뉴로 돌아갑니다.", end='\r')
            for stock in self.phone:
                if stock.name == name and stock.stat == status:  # 기기명과 상태가 같은 재고의 경우 먼저 매입한 순서대로 판매되도록 함
                    print("해당 기기의 판매가는 " + str(format(stock.price, ",")) + "원 입니다.", end=' ')
                    if int(input("구매하시겠습니까?(Yes = 1, No = 0) : ")):
                        input("결제를 진행해주세요. 결제가 끝나면 아무키나 눌러주세요... ")
                        print("기기를 받아주세요.\n\n판매해주셔서 감사합니다! 3초 후 메인 메뉴로 돌아갑니다.")
                        self.phone.remove(stock)  # del self.phone[self.phone.index(stock)]
                    else:
                        print("\n다음에 만나요! 3초 후 메인메뉴로 돌아갑니다.")
                    break
            time.sleep(3)
            ...
            for i in range(3):
                ...
                if input("ECDATA : ") == ''.join(self.passwdEecrypt(pidkey)):
                    while True:
                        ...
                        if opt == 1:
                            if self.phone != []:
                                for i, stock in enumerate(self.phone):  # https://www.daleseo.com/python-enumerate/
                                    print("\nSTOCK #" + str(i) + "\nNAME : " + stock.name + "\nSTATUS : " + stock.stat + "\nPRICE : " + str(format(stock.price, ",")))
                                print("\n재고 목록 출력을 완료하였습니다.")
                            else:
                                print("재고 목록이 비어있습니다.")
...
class Phone():
    def __init__(self, name="nonamed", stat="notclassified", price="0"):
